...
import { ArrowLeft, Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Heart, ListMusic, Music } from "lucide-react";
...
export default function Playlist() {
  ...
  return (
    <main className="relative flex min-h-screen flex-col bg-gradient-to-br from-pink-50 via-purple-50 to-blue-50 selection:bg-pink-100 selection:text-pink-600 font-sans">
      ...
      <div className="flex items-center justify-between p-6 z-10 sticky top-0 bg-white/30 backdrop-blur-md border-b border-white/20 shadow-sm">
        ...
        <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-purple-600 flex items-center gap-2">
            <ListMusic size={24} className="text-pink-500"/> My Playlist
        </h1>
                    ...
                    return (
                        ...
                        >
                            <div className="w-8 text-center text-sm font-medium text-gray-400">
                                {isActive && isPlaying ? (
                                    <div className="flex justify-center gap-0.5 h-3 items-end">
                                        <div className="w-1 bg-pink-500 animate-[bounce_1s_infinite] h-full"></div>
                            ...
                            <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 rounded-lg ${track.color} flex items-center justify-center text-gray-500 shadow-sm`}>
                                    <Music size={18} />
                                </div>
                                <div>
                                    <div className={`font-semibold ${isActive ? 'text-pink-600' : 'text-gray-800'}`}>
                                        {track.title}
                                    </div>
                            ...
                            <div className="w-8 text-center">
                                <button className={`text-gray-300 hover:text-pink-500 transition-colors ${isActive ? 'text-pink-500' : ''}`}>
      ...
      <div className="fixed bottom-0 left-0 right-0 h-24 bg-white/80 backdrop-blur-xl border-t border-white/50 shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.05)] px-6 flex items-center justify-between z-50">
        ...
        <div className="flex items-center gap-4 w-1/3 min-w-[180px]">
             <div className={`w-14 h-14 rounded-xl ${currentTrack.color} flex items-center justify-center shadow-sm animate-pulse`}>
                <Music size={24} className="text-gray-600" />
            </div>
            <div className="overflow-hidden">
                <h3 className="font-bold text-gray-800 truncate leading-tight">{currentTrack.title}</h3>
        ...
        <div className="flex flex-col items-center gap-1 w-1/3 max-w-md">
            <div className="flex items-center gap-6">
                <button onClick={handlePrev} className="text-gray-400 hover:text-gray-800 transition-transform active:scale-95">
                ...
                <button onClick={handleNext} className="text-gray-400 hover:text-gray-800 transition-transform active:scale-95">
