...
export default function Home() {
	const socialLinks = [
		{
			name: "Instagram",
			href: "https://www.instagram.com/oii.seek",
			icon: <Instagram size={20} />,
			color: "hover:bg-pink-100 hover:text-pink-600",
		},
		{
			name: "Linkedin",
			href: "https://www.linkedin.com/in/moyshik",
			icon: <Linkedin size={20} />,
			color: "hover:bg-blue-100 hover:text-blue-500",
		},
		{
			name: "GitHub",
			href: "https://github.com/moyshik7",
			icon: <Github size={20} />,
			color: "hover:bg-purple-100 hover:text-purple-600",
		},
		{
			name: "Facebook",
			href: "https://www.facebook.com/oii.seek",
			icon: <Facebook size={20} />,
			color: "hover:bg-yellow-100 hover:text-yellow-600",
		},
	];
	...
	const contentLinks = [
		...
		{
			title: "Visit My Garden",
			icon: <Flower size={16} className="text-pink-500" />,
			href: "/garden",
		},
		{
			title: "My Playlist",
	...
	return (
		<main className="relative flex min-h-screen flex-col items-center justify-center p-6 overflow-hidden selection:bg-pink-100 selection:text-pink-600">
			...
			{/* Main Card */}
			<div className="z-10 w-full max-w-md glass-panel rounded-4xl p-8 md:p-10 transform transition-all hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] duration-500">
				<div className="flex flex-col items-center text-center space-y-6">
					{/* Avatar Section */}
					<div className="relative group cursor-pointer">
						<div className="absolute -inset-1 bg-linear-to-r from-pink-300 via-purple-300 to-blue-300 rounded-full blur opacity-60 group-hover:opacity-100 transition duration-500 animate-pulse"></div>
					...
					<div className="space-y-2">
						<h1 className="text-3xl font-bold bg-clip-text text-transparent bg-linear-to-r from-pink-500 to-purple-600 font-sans tracking-tight">
							Oyshik
						</h1>
						<p className="text-gray-500 font-medium text-sm md:text-base max-w-70 mx-auto leading-relaxed">
					...
					<div className="w-full h-px bg-linear-to-r from-transparent via-pink-200 to-transparent opacity-50 my-2"></div>
					...
					<div className="w-full space-y-3">
						{contentLinks.map((item) => (
							<Link
								key={item.title}
								href={item.href}
								className="w-full py-3.5 px-5 rounded-2xl bg-white/40 hover:bg-white/80 backdrop-blur-md border border-white/50 shadow-sm transition-all duration-300 transform hover:scale-[1.02] hover:shadow-md text-gray-600 hover:text-gray-800 font-semibold text-sm flex items-center justify-between group"
							>
