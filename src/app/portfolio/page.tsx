	...
	const projects = [
		{
			title: "RedFlag",
			description:
				"A mobile app that you can use to scan barcode and check for allergens.",
		...
		},
		{
			...
			description:
				"Mental health focused content platform serving as a resource hub for articles and educational content.",
		...
		},
		{
			...
			description:
				"Auto link finder (crawler) and scraper that saves data in JSON-LD.",
		...
		},
		{
			...
			description:
				"Fully featured e-commerce app with user profile, order tracking, dashboard, merchant panel, and algorithmic search.",
		...
		},
		{
			...
			description:
				"A modern marketplace platform for students to buy and sell study notes.",
	...
	return (
		<main className="relative flex min-h-screen flex-col items-center justify-start p-6 overflow-hidden selection:bg-pink-100 selection:text-pink-600">
			...
			<div className="z-10 w-full max-w-4xl mt-10 mb-8 flex items-center justify-between">
				...
				<h1 className="text-3xl font-bold bg-clip-text text-transparent bg-linear-to-r from-pink-500 to-purple-600">
					My Portfolio
				</h1>
			...
			<div className="z-10 w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
				{projects.map((project, index) => (
					<div
						key={index}
						className="glass-panel p-6 rounded-4xl hover:shadow-[0_15px_30px_-10px_rgba(0,0,0,0.1)] transition-all duration-300 transform hover:-translate-y-1 group"
					>
						<div className="flex items-start justify-between mb-4">
							<div
								className={`p-3 rounded-2xl ${project.color} group-hover:scale-110 transition-transform duration-300`}
							>
								{project.icon}
							</div>
							<Link
						...
						<div className="flex flex-wrap gap-2">
							{project.tags.map((tag) => (
								<span
									key={tag}
									className="px-3 py-1 bg-white/60 rounded-full text-xs font-semibold text-gray-500 border border-white/50"
								>
									{tag}
								</span>
							))}
