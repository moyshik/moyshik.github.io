...
export default function Garden() {
	...
	const getInitialFlowers = (): FlowerType[] => {
		...
		for (let i = 0; i < 5; i++) {
			initialFlowers.push({
				...
				y: Math.random() * 30 + 60, // 60-90% height (bottom area)
				type: types[Math.floor(Math.random() * types.length)],
				scale: 0,
				rotation: Math.random() * 20 - 10,
				age: 0,
		...
		idCounterRef.current = localId;
		return initialFlowers;
	};

	const [flowers, setFlowers] = useState<FlowerType[]>(() => getInitialFlowers());
	...
	return (
		<main
			ref={containerRef}
			onClick={handlePlant}
			className="relative select-none w-full h-screen overflow-hidden bg-linear-to-b from-sky-200 via-sky-100 to-green-100 cursor-cell selection:bg-pink-100"
		>
			{/* Sun & Clouds */}
			...
			<div className="absolute top-6 right-6 z-50 flex flex-col items-end gap-2 pointer-events-none">
				<div className="bg-white/60 backdrop-blur-md px-4 py-2 rounded-2xl shadow-sm text-center">
					<h1 className="text-xl font-bold bg-clip-text text-transparent bg-linear-to-r from-green-600 to-teal-500">
						Zen Garden
					</h1>
			...
			<div className="absolute bottom-0 w-full h-[40%] bg-linear-to-t from-green-200/50 to-transparent pointer-events-none"></div>
				...
				<div className="relative group">
					...
					<div className="absolute -top-4 left-1/2 -translate-x-1/2 text-red-500 drop-shadow-sm">
						<Flower
							...
							fill="currentColor"
							strokeWidth={1.5}
							className="text-red-700 group-hover:rotate-12 transition-transform"
						/>
					</div>
				</div>
			);
